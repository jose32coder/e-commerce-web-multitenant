...
export function OrderSummary({
  ...
  const subtotalConverted = convertPrice(
    subtotalValue,
    "USD",
    targetCurrency,
    exchange_rates,
  ...
  return (
    <div className="sticky top-8">
      <div className="h-auto bg-white border border-zinc-100 rounded-md p-8 shadow-xl shadow-zinc-200/50">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[12px] font-bold text-ink mt-1">
                      {currencySymbol}
                      {formatPrice(itemTotalPriceConverted, targetCurrency)}
                    </p>
                  </div>
                </div>
              );
          ...
          <div className="mt-6 p-4 bg-red-50 border border-red-100 rounded-lg space-y-2">
            ...
            <ul className="text-[11px] text-red-700 space-y-1 font-medium">
              {stockProblems.map((p, i) => (
                <li key={i}>
                  • {p.name} {p.variant ? `(${p.variant})` : ""}: Solo quedan{" "}
                  {p.available} unidades.
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="mt-8 pt-8 border-t border-zinc-100 space-y-4">
          <div className="flex justify-between items-center text-[11px] font-bold uppercase tracking-[0.15em] text-zinc-400">
            ...
            <span className="text-ink">
              {currencySymbol}
              {formatPrice(subtotalConverted, targetCurrency)}
            </span>
          </div>
          ...
          {!isPickup && (
            <div className="flex justify-between items-center text-[11px] font-bold uppercase tracking-[0.15em] text-zinc-400">
              ...
              <span
                className={cn(isFree ? "text-emerald-500" : "text-amber-500")}
              >
                {isFree
                  ? "Gratis"
                  : shippingPaymentType === "cod"
          ...
          <div className="pt-4 flex justify-between items-center text-ink">
            ...
            <span className="text-2xl font-black">
              {currencySymbol}
              {formatPrice(totalConverted, targetCurrency)}
            </span>
          </div>
        </div>
